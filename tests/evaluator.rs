...
fn test_evaluator() {
    ...
    assert_eq!(
        evaluate("if (false) { 10 } else { 20 }"),
        Some(Object::Integer(20))
    );
    assert_eq!(
        evaluate("if (1) { 10 } else { 20 }"),
    ...
    assert_eq!(
        evaluate("1 + true"),
        Some(Object::Error(
            "type mismatch: Integer(1), \"+\", Bool(true)".to_string()
        ))
    );
    assert_eq!(
        evaluate("if (10 > 1) { false + true; }"),
        Some(Object::Error(
            "unknown operator: Bool(false), \"+\", Bool(true)".to_string()
        ))
    );
    assert_eq!(evaluate("let a = 5; a;"), Some(Object::Integer(5)));
    assert_eq!(evaluate("let a = 5 * 5; a;"), Some(Object::Integer(25)));
    assert_eq!(
        evaluate("let a = 5; let b = 6; a + b;"),
        Some(Object::Integer(11))
    );
    assert_eq!(
        evaluate("let add = fn(a, b) { a + b; }; add(1, 2);"),
